...
void updatePawnCorrectionHistory(board *position, const int depth, const int diff, bool pvNode) {
    U64 pawnKey = position->pawnKey;

    int entry = PAWN_CORRECTION_HISTORY[position->side][pawnKey % CORRHIST_SIZE];

    const int scaledDiff = diff * CORRHIST_GRAIN;
    const int newWeight = (2 + pvNode) * myMIN(depth + 1, 16);
...
void updateMinorCorrectionHistory(board *position, const int depth, const int diff, bool pvNode) {
...
void update_non_pawn_corrhist(board *position, const int depth, const int diff, bool pvNode) {
...
int negamax(int alpha, int beta, int depth, board* pos, time* time, bool cutNode) {
    ...
    if (!pos->isSingularMove[pos->ply]) {
        ...
        if (!in_check && (bestMove == 0 || !getMoveCapture(bestMove)) &&
            ...
            updatePawnCorrectionHistory(pos, depth, corrhistBonus, pvNode);
            updateMinorCorrectionHistory(pos, depth, corrhistBonus, pvNode);
            update_non_pawn_corrhist(pos, depth, corrhistBonus, pvNode);
        }
