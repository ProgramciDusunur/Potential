...
int negamax(int alpha, int beta, int depth, board* pos, my_time* time, bool cutNode) {
    ...
    for (int count = 0; count < moveList->count; count++) {
        ...
        if (!rootNode && notTactical && isNotMated) {
            ...
            // Futility Pruning
            if (lmrDepth <= FP_DEPTH && !pvNode && !in_check && (static_eval + FUTILITY_PRUNING_OFFSET[clamp(lmrDepth, 1, 5)]) + FP_MARGIN * lmrDepth + moveHistory / 32 + pawnHistoryValue / 64 <= alpha) {
                continue;
            }
