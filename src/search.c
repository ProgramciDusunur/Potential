...
void updatePawnCorrectionHistory(board *position, const int depth, const int diff, uint8_t tt_depth) {
    ...
    int maxWeightAndTTDepthDiff = tt_depth - 16 > 0 ? tt_depth - 16 : 0;    
    const int newWeight = 2 * myMIN(depth + 1, 16 + maxWeightAndTTDepthDiff);
...
int negamax(int alpha, int beta, int depth, board* pos, time* time, bool cutNode) {
    ...
    if (!pos->isSingularMove[pos->ply]) {
        ...
        if (!in_check && (bestMove == 0 || !getMoveCapture(bestMove)) &&
            ...
            updatePawnCorrectionHistory(pos, depth, corrhistBonus, tt_depth);
            updateMinorCorrectionHistory(pos, depth, corrhistBonus, tt_depth);
            updateMajorCorrectionHistory(pos, depth, corrhistBonus, tt_depth);
            update_non_pawn_corrhist(pos, depth, corrhistBonus, tt_depth);
        }
