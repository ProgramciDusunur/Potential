...
int negamax(int alpha, int beta, int depth, board* pos, time* time, bool cutNode) {
    ...
    int static_eval = pos->isSingularMove[pos->ply] ? pos->staticEval[pos->ply] :
                      adjustEvalWithCorrectionHistory(pos, raw_eval);
    ...
    for (int count = 0; count < moveList->count; count++) {
        ...
        // search
        if (!rootNode && depth >= 6 && currentMove == tt_move && !pos->isSingularMove[pos->ply] &&
