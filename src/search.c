...
int negamax(int alpha, int beta, SearchStack *ss, SearchData *sd, int depth, board* position, time* time, bool cutNode) {
    ...
    if (depth >= nullMoveDepth && in_check == 0 && position->ply && static_eval >= beta) {
        ...
        score = -negamax(-beta, -beta + 1, ss + 1, sd, depth - R, position, time, !cutNode);
    ...
    for (int count = 0; count < moveList->count; count++) {
        ...
        if (moves_searched == 0)
            ...
            if (moves_searched >= lmr_full_depth_moves && depth >= lmr_reduction_limit && getMovePromoted(currentMove) == 0) {
                ...
                if (pvNode) {
                    score = -negamax(-alpha - 1, -alpha, ss + 1, sd, depth - lmrReduction, position, time, false);
                } else {
                    score = -negamax(-alpha - 1, -alpha, ss + 1, sd, depth - lmrReduction, position, time, !cutNode);
                }
...
void searchPosition(int depth, SearchStack *ss, SearchData *sd, board* position, bool benchmark, time* time) {
    ...
    for (int current_depth = 1; current_depth <= depth; current_depth++) {
        ...
        score = negamax(alpha, beta, ss, sd, current_depth, position, time, false);
