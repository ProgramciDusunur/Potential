...
int negamax(int alpha, int beta, int depth, board* position, time* time, bool cutNode) {
    ...
    if (pastStack && !in_check) {
        ...
        position->improvingRate[position->ply] = fmin(fmax(position->improvingRate[position->ply] + diff / (50 + (position->improvingRate[pastStack] * 8)), (-1.0 - cutNodeSubtraction)), 1.0);
    }
