...
/*╔═══════════════════════════════╗
  ...
  /*╔════════════════════╗
    ║ Futility Pruning   ║
    ╚════════════════════╝*/
  int FUTILITY_PRUNING_OFFSET[] = {0, 82, 71, 62, 54, 47, 41, 36, 32, 28, 25};
  int FP_DEPTH = 10;
  int FP_MARGIN = 82;
...
int negamax(int alpha, int beta, int depth, board* pos, time* time, bool cutNode) {
    ...
    for (int count = 0; count < moveList->count; count++) {
        ...
        if (!rootNode && notTactical && isNotMated) {
                ...
                // Futility Pruning
                if (lmrDepth <= FP_DEPTH && !pvNode && !in_check && (static_eval + FUTILITY_PRUNING_OFFSET[clamp(lmrDepth, 1, 10)]) + FP_MARGIN * lmrDepth <= alpha) {
                    continue;
                }
