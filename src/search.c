...
int negamax(int alpha, int beta, int depth, board* pos, time* time, bool cutNode) {
    ...
    opponent_worsening = pastStack > -1 && !in_check && pos->staticEval[pos->ply] + pos->staticEval[pos->ply-1] > 1;
    ...
    uint16_t rfpMargin = improving ? RFP_IMPROVING_MARGIN * (depth - 1) : (RFP_MARGIN - 15 * opponent_worsening) * depth;
