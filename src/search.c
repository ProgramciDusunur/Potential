...
int negamax(int alpha, int beta, int depth, board* pos, time* time, bool cutNode) {
    ...
    bool tt_pv_decision = !tt_pv || (tt_pv && tt_hit && tt_score >= beta + 90 - 15 * ((tt_depth + depth) / 2));

    // Reverse Futility Pruning
    if (!pos->isSingularMove[pos->ply] && tt_pv_decision &&
        depth <= RFP_DEPTH && !pvNode && !in_check && (!tt_hit || ttAdjustedEval != static_eval) &&
        ttAdjustedEval - rfpMargin >= beta)
        return ttAdjustedEval;

    // Null Move Pruning
    if (!pos->isSingularMove[pos->ply] && !pvNode && tt_pv_decision &&
