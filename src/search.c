...
int negamax(int alpha, int beta, int depth, board* position, time* time, bool cutNode) {
    ...
    for (int count = 0; count < moveList->count; count++) {
        ...
        if (!rootNode && isQuiet && isNotMated) {
            ...
            uint8_t lmpNoisyTTMoveDivisor = getMoveCapture(bestMove) ? (2 - (position->nmpNode && depth >= 6)) : 1;
            int lmpThreshold = (((lmpBase + (lmpMultiplier + improving) * depth * depth) - improvingFactor) + lateMoveHistoryFactor) / lmpNoisyTTMoveDivisor;
            if (legal_moves>= lmpThreshold) {
                skipQuiet = 1;
            }
            // Futility pruning
