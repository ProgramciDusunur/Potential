...
/*╔═══════════════════════════════╗
  ...
  ╚═══════════════════════════════╝*/
  TUNE_INT SEE_PIECE_VALUES[] = {88, 328, 308, 569, 1208, 0, 0};
  TUNE_INT QS_SEE_THRESHOLD = 9;
  TUNE_INT QS_FP_SEE_THRESHOLD = -8;
  TUNE_INT SEE_MOVE_ORDERING_THRESHOLD = -63;
  TUNE_INT SEE_QUIET_THRESHOLD = -68;
  TUNE_INT SEE_NOISY_THRESHOLD = -24;
  TUNE_INT SEE_DEPTH = 10;
  TUNE_INT MOVE_ORDER_HIST_MULT = 493;
  TUNE_INT SEE_QUIET_HIST_MULT = 86;
  TUNE_INT SEE_QUIET_HIST_DIVISOR = 13186;
  ...
  TUNE_DOUBLE LMR_TABLE_BASE_NOISY = 0.37364353955602886;
  TUNE_DOUBLE LMR_TABLE_NOISY_MULT = 135.61583684832615;
  TUNE_DOUBLE LMR_TABLE_BASE_QUIET = 0.9379994186907842;
  TUNE_DOUBLE LMR_TABLE_QUIET_MULT = 133.3398697526084;
  TUNE_INT LMR_FULL_DEPTH_MOVES = 2;
  TUNE_INT LMR_REDUCTION_LIMIT = 3;
  TUNE_INT DEEPER_LMR_MARGIN = 35;  
  TUNE_INT QUIET_HISTORY_LMR_MULT = 1312;
  TUNE_INT QUIET_HISTORY_LMR_DIVISOR = 7417720;
  TUNE_INT QUIET_HISTORY_LMR_MINIMUM_SCALAR = 3412;
  TUNE_INT QUIET_HISTORY_LMR_MAXIMUM_SCALAR = 3138;
  TUNE_INT PAWN_HISTORY_LMR_MULT = 2061;
  TUNE_INT PAWN_HISTORY_LMR_DIVISOR = 9072535;
  TUNE_INT PAWN_HISTORY_LMR_MINIMUM_SCALAR = 3138;
  TUNE_INT PAWN_HISTORY_LMR_MAXIMUM_SCALAR = 3217;
  TUNE_INT NOISY_HISTORY_LMR_MULT = 90;  
  TUNE_INT NOISY_HISTORY_LMR_DIVISOR = 1289971;
  TUNE_INT QUIET_NON_PV_LMR_SCALAR = 1033;
  TUNE_INT CUT_NODE_LMR_SCALAR = 1912;
  TUNE_INT TT_PV_LMR_SCALAR = 1114;
  TUNE_INT TT_PV_FAIL_LOW_LMR_SCALAR = 1077;
  TUNE_INT TT_CAPTURE_LMR_SCALAR = 969;
  TUNE_INT GOOD_EVAL_LMR_SCALAR = 1030;
  TUNE_INT GOOD_EVAL_LMR_MARGIN = 357;
  TUNE_INT FUTILITY_LMR_BASE = 205;
  TUNE_INT FUTILITY_LMR_MULT = 83;
  TUNE_INT FUTILITY_LMR_SCALAR = 1024;
  TUNE_INT IMPROVING_LMR_SCALAR = 1097;
  TUNE_INT IMPROVING_FAIL_HIGH_MARGIN = 85;
  TUNE_INT GIVES_CHECK_LMR_SCALAR = 945;
  TUNE_INT CUT_NODE_LMR_NO_TT_SCALAR = 1104;
  TUNE_INT TT_PV_LMR_PV_NODE_SCALAR = 522;
  ...
  /*╔═══════════════════════╗
    ║     Move Ordering     ║
    ╚═══════════════════════╝*/
  TUNE_INT MAIN_HIST_WEIGHT = 854;
  TUNE_INT CONTHIST_1_WEIGHT = 944;
  TUNE_INT CONTHIST_2_WEIGHT = 999;
  TUNE_INT CONTHIST_4_WEIGHT = 1062;
  TUNE_INT PAWN_HIST_WEIGHT = 1145;
  TUNE_INT MAIN_HIST_DIVISOR = 1157;
  TUNE_INT CONTHIST_1_DIVISOR = 1228;
  TUNE_INT CONTHIST_2_DIVISOR = 884;
  TUNE_INT CONTHIST_4_DIVISOR = 1154;
  TUNE_INT PAWN_HIST_DIVISOR = 1066;
  
  
  /*╔═══════════════════════╗
    ║ Late Move Pruning     ║
    ╚═══════════════════════╝*/
  TUNE_INT LMP_BASE = 3915;
  TUNE_INT LMP_MULTIPLIER = 2907;
  TUNE_INT LMP_HIST_LIMIT_NEG = 5790;
  TUNE_INT LMP_HIST_LIMIT_POS = 6216;
  TUNE_INT LMP_HIST_MULT = 261;
  TUNE_INT LMP_HIST_DIVISOR = 17409;

/*╔═════════════╗
  ║   Probcut   ║
  ╚═════════════╝*/
  TUNE_INT PROBCUT_BETA_MARGIN = 182;
  TUNE_INT PROBCUT_DEPTH = 5;
  TUNE_INT PROBCUT_DEPTH_SUBTRACTOR = 4;
  TUNE_INT PROBCUT_IMPROVING_MARGIN = 35;
  TUNE_INT PROBCUT_SEE_NOISY_THRESHOLD = 114;
  TUNE_INT PROBCUT_NOISY_HIST_MULT = 90;
  TUNE_INT PROBCUT_NOISY_HIST_DIVISOR = 1287464;
  TUNE_INT PROBCUT_REDUCTION_MULTIPLIER = 325;
  TUNE_INT PROBCUT_CUTNODE_SCALAR = 926;
  TUNE_INT PROBCUT_FP_BASE = 181;
  TUNE_INT PROBCUT_FP_MULT = 95;


/*╔═══════════════════╗
  ║   Small Probcut   ║
  ╚═══════════════════╝*/
  TUNE_INT SPROBCUT_BETA_MARGIN = 379;
  ...
  /*╔════════════════════╗
    ║ Futility Pruning   ║
    ╚════════════════════╝*/
  TUNE_INT FUTILITY_PRUNING_OFFSET[] = {0, 90, 44, 22, 11, 6};
  TUNE_INT FP_DEPTH = 10;
  TUNE_INT FP_MARGIN = 61;
  TUNE_INT FP_QUAD_MULT = 51394;
  TUNE_INT FP_QUAD_DIVISOR = 1041;
  TUNE_INT BNFP_MARGIN = 74;
  TUNE_INT BNFP_QUAD_MULT = 50779;
  TUNE_INT BNFP_QUAD_DIVISOR = 1012;
  TUNE_INT QUIET_HISTORY_PRUNING_MARGIN = 1960;
  TUNE_INT FP_HIST_MULT = 516;
  TUNE_INT FP_HIST_DIVISOR = 13231;
  
  
  /*╔══════════════════════════════╗
    ║ Reverse Futility Pruning     ║
    ╚══════════════════════════════╝*/
  TUNE_INT RFP_MARGIN = 60;
  TUNE_INT RFP_IMPROVING_MARGIN = 42;
  TUNE_INT RFP_CORRPLEXITY_MULT = 9347;
  TUNE_INT RFP_CORRPLEXITY_DIVISOR = 660;
  TUNE_INT RFP_DEPTH = 11;
  TUNE_INT RFP_TT_PV_BASE = 79;
  TUNE_INT RFP_TT_PV_MULT = 16;
  TUNE_INT RFP_TT_PV_TT_WEIGHT = 1056;
  ...
  TUNE_INT LDSE_CORRECTION_MULT = 701;
  TUNE_INT LDSE_CORRECTION_DIVISOR = 98376;
  TUNE_INT SE_TT_DEPTH_SUBTRACTOR = 3;
  // Positive Extensions
  TUNE_INT DOUBLE_EXTENSION_MARGIN = -31;
  TUNE_INT TRIPLE_EXTENSION_MARGIN = 54;
  TUNE_INT QUADRUPLE_EXTENSION_MARGIN = 97;
  TUNE_INT MULTI_LOW_DEPTH_EXT_MARGIN = -13;
  TUNE_INT QUADRUPLE_EXT_NOISY_BONUS = 150;
  TUNE_INT SE_CORRECTION_MULT = 1007;
  TUNE_INT SE_CORRECTION_DIVISOR = 2755631;
  // Negative Extensions
  TUNE_INT TRIPLE_EXT_HIST_MULT = 868;
  TUNE_INT TRIPLE_EXT_HIST_DIVISOR = 520640;
  TUNE_INT TRIPLE_EXT_NOISY_BONUS = 91;
  ...
  /*╔══════════════════════════════╗
    ║      Aspiration Windows      ║
    ╚══════════════════════════════╝*/
  TUNE_INT ASP_WINDOW_BASE = 1687;
  TUNE_INT ASP_WINDOW_DIVISOR = 597;
  TUNE_INT ASP_WINDOW_MIN_DEPTH = 4;
  TUNE_DOUBLE ASP_WINDOW_MULTIPLIER = 1.8447813172637662;
...
/*╔═══════════════════════╗
  ...
  // Quiet History
  TUNE_INT QUIET_HIST_BONUS_BASE = 7;
  TUNE_INT QUIET_HIST_BONUS_DEPTH = 137;
  TUNE_INT QUIET_HIST_BONUS_MAX = 3668;
  TUNE_INT QUIET_HIST_MALUS_BASE = 9;
  TUNE_INT QUIET_HIST_MALUS_DEPTH = 241;
  TUNE_INT QUIET_HIST_FAILED_LOW_BONUS = 260;
  TUNE_INT QUIET_HIST_FAILED_LOW_MALUS = 190;
  TUNE_INT QUIET_HIST_MALUS_MAX = 3699;
  TUNE_INT HISTORY_RED_MULT = 1145;
  TUNE_INT HISTORY_RED_DIVISOR = 16700156;
  TUNE_INT CONTHIST_MULT = 1164;

  // Continuation History
  TUNE_INT CONTHIST_BONUS_BASE = 8;
  TUNE_INT CONTHIST_BONUS_DEPTH = 157;
  TUNE_INT CONTHIST_BONUS_MAX = 3905;
  TUNE_INT CONTHIST_MALUS_BASE = 10;
  TUNE_INT CONTHIST_MALUS_DEPTH = 210;
  TUNE_INT CONTHIST_FAILED_LOW_BONUS = 203;
  TUNE_INT CONTHIST_FAILED_LOW_MALUS = 147;
  TUNE_INT CONTHIST_MALUS_MAX = 4149;

  // Pawn History
  TUNE_INT PAWNHIST_BONUS_BASE = 10;
  TUNE_INT PAWNHIST_BONUS_DEPTH = 179;
  TUNE_INT PAWNHIST_BONUS_MAX = 4452;
  TUNE_INT PAWNHIST_MALUS_BASE = 7;
  TUNE_INT PAWNHIST_MALUS_DEPTH = 236;
  TUNE_INT PAWNHIST_FAILED_LOW_BONUS = 211;
  TUNE_INT PAWNHIST_FAILED_LOW_MALUS = 252;
  TUNE_INT PAWNHIST_MALUS_MAX = 3449;

  // Capture History
  TUNE_INT CAPTHIST_BONUS_BASE = 5;
  TUNE_INT CAPTHIST_BONUS_DEPTH = 182;
  TUNE_INT CAPTHIST_BONUS_MAX = 4290;
  TUNE_INT CAPTHIST_MALUS_BASE = 11;
  TUNE_INT CAPTHIST_MALUS_DEPTH = 253;
  TUNE_INT CAPTHIST_MALUS_MAX = 4421;  

  // Bad Quiet Index Scaling
  TUNE_INT BAD_QUIET_INDEX_SCALE = 24;
...
void init_lmr_table(void) {
    for (int depth = 1; depth < maxPly; ++depth) {
        for (int moves = 1; moves < maxPly; ++moves) {
            ...
            LMR_TABLE[0][depth][moves] = LMR_TABLE_BASE_NOISY + log(depth) * log(moves) * (LMR_TABLE_NOISY_MULT / 128.0) / 3.76; // noisy/tactical
            LMR_TABLE[1][depth][moves] = LMR_TABLE_BASE_QUIET + log(depth) * log(moves) * (LMR_TABLE_QUIET_MULT / 128.0) / 2.32; // quiet
        }
    }
}
...
int scoreMove(uint16_t move, ThreadData *t, SearchStack *ss) {    
    ...
    if (getMoveCapture(move)) {
        ...
        captureScore += SEE(&t->pos, move, SEE_MOVE_ORDERING_THRESHOLD - (move_history * MOVE_ORDER_HIST_MULT) / 16384) ? GOOD_CAPTURE_SCORE : BAD_CAPTURE_SCORE;
...
int quiescence(int alpha, int beta, ThreadData *t, my_time* time, SearchStack *ss) {
    ...
    writeHashEntry(position->hashKey, bestScore, raw_eval, bestMove, 0, hashFlag, tt_pv, position, position->fifty);
...
int negamax(int alpha, int beta, int depth, ThreadData *t, my_time* time, SearchStack *ss, bool predicted_cut_node) {
    ...
    if (!pvNode && !in_check && depth >= PROBCUT_DEPTH && abs(beta) < mateValue  && !ss->singular_move &&
        (!tt_hit || tt_depth + 3 < depth || tt_score >= probcut_beta)) {
            ...
            for (int count = 0; count < capture_promos->count; count++) {
                ...
                if (probcut_value >= probcut_beta) {
                    writeHashEntry(pos->hashKey, probcut_value, raw_eval, move, probcut_depth, hashFlagAlpha, tt_pv, pos, pos->fifty);
    ...
    while ((currentMove = get_next_move(&mp, move_scores, pos, t, ss)) != 0) {
        ...
        int pawnHistoryValue = notTactical ? t->shared_history->pawnHistory[pos->pawnKey & t->shared_history->pawn_history_mask][pos->mailbox[getMoveSource(currentMove)]][getMoveTarget(currentMove)] : 0;
        ...
        if (lmrDepth <= SEE_DEPTH && moves_seen > 0 && isNotMated && !SEE(pos, currentMove, seeThreshold))
            continue;
        ...
        if (pos->ply < depth * 2 && !rootNode && depth >= SE_DEPTH + tt_pv && currentMove == tt_move && !ss->singular_move &&
            ...
            // Multicut
                ...
                if (!in_check && singularBeta > static_eval) {
                    const int bonus = clamp((singularBeta - static_eval) * singularDepth * 64 / 1024, -CORRHIST_LIMIT / 4, CORRHIST_LIMIT / 4);
                    ...
                    update_non_pawn_corrhist(t, depth, bonus);
                }
