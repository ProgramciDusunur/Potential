...
int negamax(int alpha, int beta, int depth, board* position, time* time, bool cutNode) {
    ...
    double improvingPositiveClamp = bestMove && pvNode && depth >= 3 ? 0.125 : 0;
    ...
    if (pastStack && !in_check) {
        ...
        position->improvingRate[position->ply] = fmin(fmax(position->improvingRate[position->ply] + diff / 50, (-1.0 - cutNodeSubtraction)), (1.0 + improvingPositiveClamp));
    }
