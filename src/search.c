...
int scoreMove(int move, board* position, SearchStack *ss) {
    ...
    if (getMoveCapture(move)) {
        ...
        return mvvLva[getMovePiece(move)][target_piece] + getContinuationHistoryScore(ss, move) + 1000000000;
...
int quiescence(int alpha, int beta, board* position, SearchStack *ss, int negamaxScore, time* time, bool improving) {
    ...
    for (int count = 0; count < moveList->count; count++) {
        ...
        // score current move
        score = -quiescence(-beta, -alpha, position, ss, score, time, improving);
...
int negamax(int alpha, int beta, int depth, board* position, SearchStack *ss, time* time, bool cutNode) {
    ...
    if (depth == 0)
        ...
        return quiescence(alpha, beta, position, ss, score, time, improving);
    ...
    if (depth >= nullMoveDepth && in_check == 0 && position->ply && static_eval >= beta) {
        ...
        score = -negamax(-beta, -beta + 1, depth - R, position, ss, time, !cutNode);
    ...
    for (int count = 0; count < moveList->count; count++) {
        ...
        if (moves_searched == 0)
            ...
            score = -negamax(-beta, -alpha, depth - 1, position, ss, time, false);
            ...
            if (moves_searched >= lmr_full_depth_moves &&
                ...
                if (pvNode) {
                    score = -negamax(-alpha - 1, -alpha, depth - lmrReduction, position, ss, time, false);
                } else {
                    score = -negamax(-alpha - 1, -alpha, depth - lmrReduction, position, ss, time, !cutNode);
                }
            ...
            if (score > alpha) {
                ...
                score = -negamax(-alpha - 1, -alpha, depth - 1, position, ss, time, false);
...
void searchPosition(int depth, board* position, SearchStack *ss, bool benchmark, time* time) {
    ...
    for (int current_depth = 1; current_depth <= depth; current_depth++) {
        ...
        score = negamax(alpha, beta, current_depth, position, ss, time, false);
