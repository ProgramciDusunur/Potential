...
void prefetch_cont_corrhist(ThreadData *t, SearchStack *ss) {
    if (t->pos.ply >= 1 && ss->move) {
        SearchStack *prev = ss - 1;
        if (prev->move) {
            __builtin_prefetch(&t->shared_history->contCorrhist
                [prev->piece][getMoveTarget(prev->move)]
                [ss->piece][getMoveTarget(ss->move)]);
        }
    }
}
