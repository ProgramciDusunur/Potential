...
int extractDepths(char* suitFen, int* depths) {
    char* suitFenCopy = strdup(suitFen);
    if (suitFenCopy == NULL) {
        fprintf(stderr, "Memory allocation error!\n");
        return 0; // Return 0 on error
    }

    char* token = strtok(suitFenCopy, ";");
    int depthIndex = 0;

    while (token != NULL) {
        if (strstr(token, "D") != NULL) {
            depths[depthIndex] = atoi(token + 2);
            depthIndex++;
        }
        token = strtok(NULL, ";");
    }
    ...
    free(suitFenCopy);
    return depthIndex; // Return the number of depths
}

// Performs perft tests for all suit positions
void perftSuite() {
    int depths[6];
    int correctCount = 0;
    int totalCount = 126;
    ...
    for (int i = 0; i < totalCount; i++) {
        board position;
        int depthCount = extractDepths(perftSuitFens[i], depths);

        parseFEN(perftSuitFens[i], &position);
