...
void clear_histories(void) {
    ...
    // Re-allocate shared histories dynamically to preserve NUMA first-touch policy on clearing
    ...
    // CRITICAL: Immediately update each thread's pointer to the newly allocated SharedHistory to avoid dangling pointers!
