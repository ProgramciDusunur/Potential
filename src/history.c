...
void update_quiet_histories(board *pos, uint16_t bestMove, int depth, moves *badQuiets) {
    int16_t from = getMoveSource(bestMove);
    int16_t to = getMoveTarget(bestMove);
    int16_t piece = pos->mailbox[from];
    bool threatSource = is_square_threatened(pos, from);
    bool threatTarget = is_square_threatened(pos, to);
    ...
    int16_t quiet_hist_score = quietHistory[pos->side][from][to][threatSource][threatTarget];
    quietHistory[pos->side][from][to][threatSource][threatTarget] += scaledBonus(quiet_hist_score, bonus, maxQuietHistory);    

    int16_t pawn_hist_score = pawnHistory[pos->pawnKey % 2048][pos->mailbox[from]][to];
    pawnHistory[pos->pawnKey % 2048][pos->mailbox[from]][to] += scaledBonus(pawn_hist_score, bonus, maxPawnHistory);
    ...
    for (int index = 0; index < badQuiets->count; index++) {
        ...
        int16_t badQuietFrom = getMoveSource(badQuiets->moves[index]);
        int16_t badQuietTo = getMoveTarget(badQuiets->moves[index]);

        bool badQuietThreatSource = is_square_threatened(pos, badQuietFrom);
        bool badQuietThreatTarget = is_square_threatened(pos, badQuietTo);

        int16_t badQuietScore = quietHistory[pos->side][badQuietFrom][badQuietTo][badQuietThreatSource][badQuietThreatTarget];

        quietHistory[pos->side][badQuietFrom][badQuietTo][badQuietThreatSource][badQuietThreatTarget] +=
        scaledBonus(badQuietScore, -bonus, maxQuietHistory);
        updateAllCH(pos, badQuiets->moves[index], -bonus, quiet_hist_score);
        pawnHistory[pos->pawnKey % 2048][pos->mailbox[badQuietFrom]][badQuietTo] += scaledBonus(pawn_hist_score, -bonus, maxPawnHistory);
    }
}
