...
void update_pawn_correction_hist(ThreadData *t, const int depth, const int diff) {
    ...
    int16_t *entry = &thread_pool.corrhist.pawn_corrhist[t->pos.side][t->pos.pawnKey & (CORRHIST_SIZE - 1)];
...
void update_minor_correction_hist(ThreadData *t, const int depth, const int diff) {
    ...
    int16_t *entry = &thread_pool.corrhist.minor_corrhist[t->pos.side][t->pos.minorKey & (CORRHIST_SIZE - 1)];
    apply_corrhist_update(entry, scaledDiff, newWeight);
}
...
void update_major_correction_hist(ThreadData *t, const int depth, const int diff) {
    ...
    int16_t *entry = &thread_pool.corrhist.major_corrhist[t->pos.side][t->pos.majorKey & (CORRHIST_SIZE - 1)];
    apply_corrhist_update(entry, scaledDiff, newWeight);
}
...
void update_non_pawn_corrhist(ThreadData *t, const int depth, const int diff) {    
    ...
    int16_t *white_ptr = &thread_pool.corrhist.non_pawn_corrhist[white][side][t->pos.whiteNonPawnKey & mask];
    apply_corrhist_update(white_ptr, scaledDiff, newWeight);
    
    int16_t *black_ptr = &thread_pool.corrhist.non_pawn_corrhist[black][side][t->pos.blackNonPawnKey & mask];
...
void update_king_rook_pawn_corrhist(ThreadData *t, const int depth, const int diff) {
    ...
    int16_t *entry = &thread_pool.corrhist.krp_corrhist[t->pos.side][t->pos.krpKey & (CORRHIST_SIZE - 1)];
    apply_corrhist_update(entry, scaledDiff, newWeight);
}
...
void update_single_cont_corrhist_entry(ThreadData *t, const int pliesBack, const int scaledDiff, const int newWeight, SearchStack *ss) {
    ...
    if (m1 && m2) {        
        int16_t *entry_ptr = &thread_pool.corrhist.contCorrhist[prev->piece][getMoveTarget(m1)]
...
static inline int adjust_single_cont_corrhist_entry(ThreadData *t, const int pliesBack, SearchStack *ss) {
    ...
    if (m1 && m2) {
        return thread_pool.corrhist.contCorrhist[prev->piece][getMoveTarget(m1)]
                               [ss->piece][getMoveTarget(m2)];
    }
    return 0;
}
...
int adjust_eval_with_corrhist(ThreadData *t, int rawEval, SearchStack *ss) {       
    ...
    // Batch memory access    
    int adjust = thread_pool.corrhist.pawn_corrhist[side][t->pos.pawnKey & mask]
               + thread_pool.corrhist.minor_corrhist[side][t->pos.minorKey & mask]
               + thread_pool.corrhist.major_corrhist[side][t->pos.majorKey & mask]
               + thread_pool.corrhist.krp_corrhist[side][t->pos.krpKey & mask]
               + thread_pool.corrhist.non_pawn_corrhist[white][side][t->pos.whiteNonPawnKey & mask]
               + thread_pool.corrhist.non_pawn_corrhist[black][side][t->pos.blackNonPawnKey & mask]
...
int get_correction_value(ThreadData *t, SearchStack *ss) {
    ...
    const int pawn_correction = thread_pool.corrhist.pawn_corrhist[side][t->pos.pawnKey & mask];
    const int minor_correction = thread_pool.corrhist.minor_corrhist[side][t->pos.minorKey & mask];
    const int major_correction = thread_pool.corrhist.major_corrhist[side][t->pos.majorKey & mask];
    const int krp_correction = thread_pool.corrhist.krp_corrhist[side][t->pos.krpKey & mask];
    const int white_non_pawn_correction = thread_pool.corrhist.non_pawn_corrhist[white][side][t->pos.whiteNonPawnKey & mask];
    const int black_non_pawn_correction = thread_pool.corrhist.non_pawn_corrhist[black][side][t->pos.blackNonPawnKey & mask];
...
void clear_histories(void) {
    ...
    memset(&thread_pool.corrhist, 0, sizeof(CorrectionHistory));
}
