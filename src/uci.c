...
void scaleTime(time* time, uint8_t bestMoveStability) {
    double bestMoveScale[6] = {2.43, 1.35, 1.09, 0.88, 0.68, 0.56};
    time->softLimit =
...
void goCommand(char *command, board* position, time* time) {
    ...
    if (time->time != -1) {
        ...
        time->maxTime = MAX(1, time->time * MAX_TIME_MULTIPLIER);
        time->baseSoft = MIN(baseTime * SOFT_LIMIT_MULTIPLIER, time->maxTime);
        time->hardLimit =
                time->starttime + MIN(baseTime * HARD_LIMIT_MULTIPLIER, time->maxTime);
        time->softLimit =
                time->starttime + MIN(baseTime * SOFT_LIMIT_MULTIPLIER, time->maxTime);
    } else {
        time->timeset = 0;
    }
