...
void init_qs_mp(QSearchPicker *mp, uint16_t tt_move) {
    mp->tt_move = tt_move;
    mp->CURRENT_STAGE = STAGE_QS_EVASIONS_TT;
    mp->index = 0;
    mp->good_noisy_index = 0;
    mp->bad_noisy_index = 0;
    mp->quiet_index = 0;
...
uint16_t qs_get_next_move(QSearchPicker *mp, int *move_scores, board *pos, ThreadData *t, SearchStack *ss, bool evasions_available) {
    switch (mp->CURRENT_STAGE) {
        ...
        // fallthrough
        case STAGE_QS_EVASIONS_GEN_NOISY:
            mp->CURRENT_STAGE = STAGE_QS_EVASIONS_GOOD_NOISY;
            noisyGenerator(&mp->good_noisy, pos);
            init_quiescence_scores(&mp->good_noisy, move_scores, pos);
            mp->good_noisy_index = 0;
        // fallthrough
        case STAGE_QS_EVASIONS_GOOD_NOISY:
            while (mp->good_noisy_index < mp->good_noisy.count) {
                pick_next_move(mp->good_noisy_index, &mp->good_noisy, move_scores);
            ...
            mp->CURRENT_STAGE = STAGE_QS_EVASIONS_GEN_QUIET;
        // fallthrough
        case STAGE_QS_EVASIONS_GEN_QUIET:
            mp->CURRENT_STAGE = STAGE_QS_EVASIONS_QUIET;
            if (evasions_available) {
                quietGenerator(&mp->quiet, pos);
                init_move_scores(&mp->quiet, move_scores, mp->tt_move, t, ss);
                mp->quiet_index = 0;
            }
        // fallthrough
        case STAGE_QS_EVASIONS_QUIET:
            if (evasions_available) {
                while (mp->quiet_index < mp->quiet.count) {
                    pick_next_move(mp->quiet_index, &mp->quiet, move_scores);
            ...
            mp->CURRENT_STAGE = STAGE_QS_EVASIONS_BAD_NOISY;
        // fallthrough
        case STAGE_QS_EVASIONS_BAD_NOISY:
            if (mp->bad_noisy_index < mp->bad_noisy.count) {
                uint16_t candidate_move = mp->bad_noisy.moves[mp->bad_noisy_index];
                mp->bad_noisy_index++;
                return candidate_move;
